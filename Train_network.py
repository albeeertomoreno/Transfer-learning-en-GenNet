...
def train_model(args):
    ...
    if os.path.exists(args.resultpath + '/bestweights_job.h5') and not(args.resume):
        ...
        # Verificar si el archivo train_log.csv existe
        if os.path.exists(args.resultpath + "/train_log.csv"):
            log_file = pd.read_csv(args.resultpath + "/train_log.csv")
            initial_epoch = len(log_file)
        # añado la posibilidad de entrenar sin el trainlogcsv
        else:
            print("train_log.csv no encontrado, comenzando desde la primera época")
            initial_epoch = 0  # Comenzar desde la primera época si no existe el archivo de log
        ...
        model.load_weights(args.resultpath + '/bestweights_job.h5', by_name=True) # intoduzco una a una la capa donde quiero hacer el transfer
    ...
    if args.regression:
        ...
        # Crear y guardar la gráfica de validación
        fig_val = plt.figure()
        fpr_val, tpr_val, _ = roc_curve(yval, pval)
        ...
        # Crear y guardar la gráfica de prueba
        fig_test = plt.figure()
